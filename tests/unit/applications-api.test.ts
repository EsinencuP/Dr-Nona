...
import {
  ...
  type ApplicationsHandlerDependencies,
...
function handler(
  serviceResult: ApplicationServiceResult,
  overrides: ApplicationsHandlerDependencies = {}
) {
    ...
    expect(
      (
        await handler(result)(
          request(validBody, { headers: { Origin: "https://evil.test" } })
        )
      ).status
    ).toBe(403);
  });
