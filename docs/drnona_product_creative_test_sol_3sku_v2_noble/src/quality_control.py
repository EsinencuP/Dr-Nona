...
def scene_detail_score(image: Image.Image) -> float:
    """Estimate whether a generated plate contains photographic material detail."""
    gray = np.asarray(image.convert("L").resize((256, 256)), dtype=np.uint8)
    histogram = np.bincount(gray.ravel(), minlength=256).astype(np.float64)
    probabilities = histogram[histogram > 0] / histogram.sum()
    entropy = float(-(probabilities * np.log2(probabilities)).sum()) / 8.0
    dx = np.abs(np.diff(gray.astype(np.float32), axis=1)).mean() / 255.0
    dy = np.abs(np.diff(gray.astype(np.float32), axis=0)).mean() / 255.0
    edge_component = min(1.0, (dx + dy) * 7.5)
    return round(max(0.0, min(1.0, entropy * 0.72 + edge_component * 0.28)), 4)
...
def hash_similarity(hash_a: str, hash_b: str) -> float:
    if not hash_a or not hash_b or len(hash_a) != len(hash_b):
        return 0.0
    distance = (int(hash_a, 16) ^ int(hash_b, 16)).bit_count()
    return round(1.0 - distance / (len(hash_a) * 4), 4)
