...
def _asset_direction(asset_type: str, scene_plan: dict[str, Any]) -> str:
    if asset_type == "hero":
        return (
            ...
            "empty studio. Use three to five coherent prop or material groups only around "
            "the outer perimeter, with foreground, midground, and background depth. Keep "
            "the central product zone and its base plane unobstructed. Every prop must "
            "reinforce this product category and the selected brand world."
        )
...
{direction}
Reserve a visible horizontal support plane at about 74 percent of canvas height.
Realistic scale, tactile materials, controlled visual richness, natural contact plane,
and generous breathing room around the protected product.
    ...
    negative = ", ".join(
        dict.fromkeys(
            ...
            + [
                ...
                "floating objects",
                "distorted geometry",
                "props behind the product placement zone",
                "props crossing the label",
