...
def compose_product(
    ...
    base_y_ratio: float | None = None,
    max_width_ratio: float | None = None,
) -> CompositeResult:
    canvas = background.convert("RGBA")
    cropped, source_bbox = crop_to_alpha(product)
    width_ratio = (
        min(max_width_ratio, 1.0 - 2 * safe_padding_ratio)
        if max_width_ratio is not None
        else 1.0 - 2 * safe_padding_ratio
    )
    max_w = int(canvas.width * width_ratio)
    ...
    if base_y_ratio is None:
        y = (canvas.height - resized.height) // 2
    else:
        y = round(canvas.height * base_y_ratio - resized.height)
