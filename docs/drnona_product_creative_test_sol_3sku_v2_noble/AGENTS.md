...
Run under GPT-5.6-sol and generate only empty background plates with built-in
Codex `imagegen`. Save them in `input/backgrounds` and use the local folder
provider. Never call an external image provider or request an API key.
