...
from .quality_control import evaluate_composite
...
class CatalogCreativePipeline:
    ...
    def _provider(self):
        ...
        if self.options.provider == "gemini":
            return GeminiImageProvider(
                max_retries=int(self.settings["budget"]["max_retries_per_request"]),
                allow_billable=self.options.allow_billable,
            )
    ...
    def _solid_background(self, size: int) -> Image.Image:
        color = ImageColor.getrgb(self.settings["canvas"]["background"])
    ...
    def _save_final_copies(
        ...
        if asset_type == "clean-catalog":
            export_thumbnail(
                image,
                bucket_dir / "01-clean-catalog-thumbnail.webp",
    ...
    def _process_product(self, record: ProductRecord, provider: Any | None) -> None:
        ...
        if generation_allowed and "clean-catalog" in scene_plan["assets"] and (
            not self.options.asset_type or self.options.asset_type == "clean-catalog"
        ):
            result = compose_product(
                product,
                self._solid_background(int(self.settings["canvas"]["catalog_size"])),
            ...
            png_path, _ = self._save_final_copies(
                record, "clean-catalog", result.image, qa["status"]
            )
            assets["clean-catalog"] = png_path.relative_to(self.root / "output").as_posix()
        ...
        if provider is not None and generation_allowed:
            for asset_type, prompt_data in prompts["items"].items():
                if self.options.asset_type and self.options.asset_type != asset_type:
                    continue
                count = 2 if self.options.pilot and asset_type in {"hero", "ingredients"} else 1
