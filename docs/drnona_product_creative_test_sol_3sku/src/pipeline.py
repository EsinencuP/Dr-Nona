...
class CatalogCreativePipeline:
    ...
    @staticmethod
    def _transparent_background(size: int) -> Image.Image:
    ...
    def _save_final_copies(
        ...
        if asset_type == "clean-cutout":
            export_thumbnail(
                image,
                bucket_dir / "01-clean-cutout-thumbnail.webp",
    ...
    def _process_product(self, record: ProductRecord, provider: Any | None) -> None:
        ...
        if generation_allowed and "clean-cutout" in scene_plan["assets"] and (
            not self.options.asset_type or self.options.asset_type == "clean-cutout"
        ):
            result = compose_product(
                product,
                self._transparent_background(
                    int(self.settings["canvas"]["catalog_size"])
                ),
            ...
            png_path, _ = self._save_final_copies(
                record, "clean-cutout", result.image, qa["status"]
            )
            assets["clean-cutout"] = png_path.relative_to(self.root / "output").as_posix()
