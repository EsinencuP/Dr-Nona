# Archived Dr. Nona Moldova frontend QA report

Archived: 2026-07-30  
Superseded by: `docs/QA_REPORT.md` and `docs/BUILD_REPORT.md`  
Reason: this report combines several worktree states and contains the obsolete
55-product catalogue count. It is retained as historical evidence only.
Commit SHA: not recorded; do not use this report for release verification  
Environment: historical local preview

Date: 2026-07-26  
Status: PASS for the deployable frontend build
...
- Selection consultation handoff: PASS for frontend context integrity; 10/10
  names, SKU and product URLs survive route navigation and refresh, while
  deletion updates both route and email payload.
- Empty selection creates no consultation CTA or empty selection email.
- Long-selection fallback: the complete list can be copied independently of
  mail-client URL limits.
...
- Language control is not exposed until the complete Romanian interface and
  content set are approved.
...
- Romanian official copy has not been invented; it needs approved translation.
  The incomplete RO mode and its persisted browser state have been removed.
- Moldova-specific phone, email and Telegram destination still need owner data.
  The site exposes the current official corporate phone and
...
- Contact transport, consent, spam protection and data-retention rules are not
  connected before the Moldova recipient is approved. The non-functional form
  has been removed; `/contactus` uses direct email and telephone links. The
  selected-product context is prepared for the official corporate email, but
  a Moldova recipient and server-confirmed delivery remain pending.
