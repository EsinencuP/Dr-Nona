...
| `TASK-03` claims cleanup | Blocked | Registry validation passes structurally, all 399 decisions remain `pending`, and affected fields are publication-blocked. | For every claim: approved/rejected decision, reviewer, ISO date and approval reference; approved or neutral replacement copy for rejected claims. |
...
| `TASK-05` popularity sorting | Implemented with owner-approved source (refreshed 2026-09-17) | Comparator uses a reviewed, expiring CRM order-frequency snapshot; the legacy `popularityRank` is ignored. RU/RO disclose official-order fallback while the real sample is insufficient. | A real threshold-eligible candidate and human-reviewed merge are needed before public demand order activates. |
...
- Claims registry gate: pass structurally with 399 pending decisions; public runtime suppresses affected fields
